...
docker run -d --network localhost --name elasticsearch -p 9200:9200 -p 9300:9300 -e "discovery.type=single-node" elasticsearch:7.9.3

docker run -d --network localhost -p 9000:9000 --name cerebro yannart/cerebro:latest

docker run -d --link <elastic-search-container-id>:elasticsearch -p 5601:5601 --network localhost docker.elastic.co/kibana/kibana:7.9.3
...
curl \
  ...
  --data-binary "@/search_template.json"

 curl \
  -H "Content-Type: application/x-ndjson" \
  -XPOST "http://localhost:9200/index_search/_bulk" \
  --data-binary "@test_search.json"
